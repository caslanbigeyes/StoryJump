...
export class BigModelProvider extends LLMProvider {
  ...
  private prepareJSON(raw: string): string {
    ...
    // 2. 去掉 JSON 行注释（LLM 有时生成 // ...）
    cleaned = cleaned.replace(/\/\/[^\n\r]*/g, '');

    // 3. 用括号计数法提取第一个完整 JSON 对象或数组
    //    （防止 LLM 在 JSON 后附加说明文字，导致 parse 失败）
...
${cleaned}`,
  ...
  async generateStoryboard(input: TaskInput): Promise<StoryboardOutput> {
    ...
    // 先建立 characterBible，再生成分镜，确保分镜生成时能引用角色设定
    const shots = await this.splitStoryboard(script, input, characterBible);
    const beats: StoryBeat[] = (shots as any).__beats__ ?? [];
  ...
  private buildStyleToken(input: TaskInput): string {
  ...
  private buildSceneContext(input: TaskInput): StoryboardOutput['meta']['scene_context'] {
    return {
  ...
  private getShotType(index: number, total: number): string {
    if (index === 0) return 'establishing_shot';
    if (index === total - 1) return 'transition_shot';
    return SHOT_TYPE_TEMPLATE[index % SHOT_TYPE_TEMPLATE.length];
  }

  private getCameraByShotType(shotType: string): ShotCamera {
    const presets: Record<string, ShotCamera> = {
  ...
  /** 清除抽象词汇：生图模型无法理解情绪/叙事词，必须删掉 */
  private cleanImagePrompt(prompt: string): string {
  ...
  /** 提取镜头状态，用于跨批次传递场景连续性 */
  private extractShotState(shot: ShotData): Record<string, unknown> {
    return {
      ...
      characters: shot.character.reduce<Record<string, string>>((acc, name) => {
...
输出格式：
{
  ...
  async splitStoryboard(
    ...
    // 把 beats 挂在第一个 shot 上传出去（由 generateStoryboard 取出）
    (shots as any).__beats__ = beats;
    return shots;
  }

  /** Beat Planner：把剧本拆解为 5-8 个有因果推进的剧情节拍 */
  private async generateBeats(script: StoryScript, input: TaskInput): Promise<StoryBeat[]> {
...
[{
  ...
  "shot_count": 3
    ...
    const beats = await this.extractJSONWithRepair<StoryBeat[]>(raw, 'generateBeats');
    return this.normalizeBeats(beats, input);
  }

  /** 修正 Beat 分配，保证 shot_count 总和严格等于 input.shot_count */
  private normalizeBeats(beats: StoryBeat[], input: TaskInput): StoryBeat[] {
  ...
  /** Beat → Action Chain：为每个 Beat 先生成原子动作序列，再用它约束 Shot 生成 */
  private async generateActionChain(
    beat: StoryBeat,
    characterBible: CharacterBible[],
    shotCount: number,
  ): Promise<Array<{ step: number; actionVerb: string; action: string }>> {
...
[{"step": 1, "actionVerb": "...", "action": "..."}]`;
    ...
    const chain = await this.extractJSONWithRepair<Array<{ step: number; actionVerb: string; action: string }>>(
      raw,
      `generateActionChain:beat${beat.beat_id}`,
    );
    // 保证数量匹配，多截少补
  ...
  /** Shot Validator：检测重复景别、连续静止、重复动作 */
  private validateShots(shots: ShotData[]): string[] {
    const errors: string[] = [];
  ...
  /** 把 Validator 错误发回 LLM，只修复失败的镜头 */
  private async repairInvalidShots(
    shots: ShotData[],
    errors: string[],
    beat: StoryBeat,
    actionChain: Array<{ step: number; actionVerb: string; action: string }>,
  ): Promise<ShotData[]> {
    ...
    if (!failedShots.length) return shots;
...
输出格式：只输出失败的镜头 JSON 数组，不要输出其他内容。`;
    ...
    const repaired = await this.extractJSONWithRepair<ShotData[]>(raw, 'repairInvalidShots');
    const repairedMap = new Map(repaired.map((s) => [s.shot_id, s]));
    return shots.map((s) => repairedMap.get(s.shot_id) ?? s);
  }

  /** 按 Beat 顺序逐节拍生成镜头，每个节拍的旁白只分配给第一个镜头 */
  private async splitShotsByBeats(
    script: StoryScript,
    beats: StoryBeat[],
    input: TaskInput,
    characterBible: CharacterBible[],
    ...
    let currentShotId = 1;
    let previousShot: ShotData | undefined;

    for (const beat of beats) {
      // 1. 先生成 Action Chain（动作链约束）
      ...
      // 4. Beat narration → 仅第一个镜头；其余静默（旁白不重复）
      // 5. 把 beat_id 写入每个 shot，供 DB 关联
      for (let i = 0; i < beatShots.length; i++) {
        beatShots[i].narration = i === 0 ? beat.narration : '';
        (beatShots[i] as any).beat_id = beat.beat_id;
      }
  ...
  /** 为单个 Beat 生成镜头序列：必须严格按照 actionChain 执行动作，不允许自由发挥 */
  private async generateShotsForBeat(
    beat: StoryBeat,
    script: StoryScript,
    input: TaskInput,
    characterBible: CharacterBible[],
...
[{
  ...
  "negative_prompt": ""
    ...
    return this.extractJSONWithRepair<ShotData[]>(raw, `generateShotsForBeat:beat${beat.beat_id}`);
  }
  ...
  async generateImagePrompts(
    ...
    if (shots.length > batchSize) {
      ...
      let previousShotState: Record<string, unknown> | undefined;
      for (let index = 0; index < shots.length; index += batchSize) {
        ...
        // 提取当前批次最后一个镜头的状态，传给下一批次，实现镜头状态机
  ...
  private async generateImagePromptsBatch(
    ...
    characterBible: CharacterBible[],
    meta: StoryboardOutput['meta'],
    previousShotState?: Record<string, unknown>,
  ): Promise<Array<{ shot_id: number; image_prompt: string; negative_prompt: string }>> {

    // 角色 Token：CHAR_{NAME} 前缀强制锚定，对齐 require2.md 角色圣经模式
    ...
    // 镜头状态机上下文：让 LLM 感知上一批的场景/光线/人物状态，保证视觉连续性
...
[{ "shot_id": 1, "image_prompt": "...", "negative_prompt": "..." }]`;
    ...
    const raw = await this.chat(systemPrompt, userMessage, 0.5);
    const results = await this.extractJSONWithRepair<Array<{ shot_id: number; image_prompt: string; negative_prompt: string }>>(
      raw,
      'generateImagePrompts',
    );

    // 后处理：按固定层级重新拼接 prompt，LLM 结果只作为补充细节，避免自由发挥破坏连续性。
    for (const result of results) {
      const shot = shots.find((s) => s.shot_id === result.shot_id);
      if (shot) {
        result.image_prompt = this.composeControlledImagePrompt(
          shot,
          characterBible,
          meta,
          result.image_prompt,
